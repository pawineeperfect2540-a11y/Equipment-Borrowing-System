...
function initEmployeeAutocomplete(){
  ...
  function renderSuggestions(q){
    ...
    const matches = EMPLOYEES.filter(e => e.name.toLowerCase().includes(ql) || e.nick.toLowerCase().includes(ql)).slice(0,8);
    if(matches.length === 0){
      list.innerHTML = `<div class="ac-empty">ไม่พบชื่อนี้ในระบบ — พิมพ์ต่อได้ ระบบจะใช้ชื่อที่พิมพ์</div>`;
    }else{
      list.innerHTML = matches.map((e,i)=>`
        <div class="ac-item" data-idx="${i}" onclick="selectEmployee('${e.name.replace(/'/g,"\\'")}','${e.nick.replace(/'/g,"\\'")}')">
          <div class="ac-name">${highlightMatch(e.name,q)} <span style="font-weight:400;color:var(--text-soft)">(${highlightMatch(e.nick,q)})</span></div>
          <div class="ac-dept">${escapeHtml(e.dept)}</div>
        </div>
      `).join("");
    }
...
function selectEmployee(name, nick){
  ...
  try{
    ...
    if(snap.empty){
      ...
      DEFAULT_EMPLOYEES.forEach(emp=>{
        ...
        batch.set(ref, { name: emp.name, nick: emp.nick, dept: emp.dept || "", createdAt: Date.now() });
      });
      await batch.commit();
...
function renderEmployeeManageList(){
  ...
  box.innerHTML = list.map(e=>`
    <div class="emprow">
      <div class="info"><b>${escapeHtml(e.name)}</b> (${escapeHtml(e.nick)})<div class="dept">${escapeHtml(e.dept||"-")}</div></div>
...
async function addEmployeeRecord(){
  ...
  try{
    await db.collection("employees").add({ name, nick, dept, createdAt: Date.now() });
...
function pdfSigBlockHtml(label, sigDataUrl, name, dateStr){
  return `
    <div style="text-align:center;">
      ...
      <div style="height:60px;display:flex;align-items:flex-end;justify-content:center;margin-top:8px;">
        ${sigDataUrl ? `<img src="${sigDataUrl}" style="max-height:60px;max-width:200px;">` : ""}
      </div>
      <div style="border-top:1px solid #8a8a8a;margin-top:6px;padding-top:8px;">
...
function buildPdfHtml(rec){
  ...
  return `
  <div style="width:794px;padding:44px;box-sizing:border-box;background:#fff;font-family:'Sarabun',sans-serif;color:#161616;line-height:1.7;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-top:26px;border-bottom:2px solid #161616;padding-bottom:10px;">
      <div style="font-weight:700;font-size:21px;">${title}</div>
      <div style="font-size:14px;">วันที่&nbsp; ${escapeHtml(formatDate(rec.dateOut))}</div>
    </div>

    <div style="margin-top:18px;font-size:14px;">บริษัท เพอร์เฟค กรุ๊ป อินเตอร์เทรดดิ้ง จำกัด</div>

    <div style="margin-top:14px;">${pdfFieldHtml("ชื่อ-นามสกุล", rec.employeeName)}</div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:26px;font-size:13.5px;">
      <thead>
    ...
    <div style="margin-top:22px;font-size:12.5px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:40px;font-size:13.5px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:36px;font-size:13.5px;">
    ...
    <div style="display:flex;justify-content:space-between;margin-top:34px;font-size:10.5px;color:#999;">
...
async function exportPDF(id){
  ...
  while(renderedY < canvas.height){
    ...
    pdf.addImage(imgData, "PNG", 0, 0, 210, sliceHeight / pxPerMM);
